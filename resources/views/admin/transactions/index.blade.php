                ...
                <table class="table-modern">
                    ...
                    <tbody>
                        @foreach ($transactions as $t)
                            <tr onclick="window.location='{{ route('admin.transactions.show', $t->id) }}'" class="cursor-pointer hover:bg-gray-50 transition-colors group">
                                <td>
                                ...
                                <td>
                                    @if($t->payment_proof)
                                        <button onclick="event.stopPropagation(); showProof('{{ asset('storage/' . $t->payment_proof) }}')"
                                    ...
                                    @if($t->status === 'Pending' && $t->payment_proof)
                                        <div class="flex items-center justify-center gap-2">
                                            <form action="{{ route('admin.transactions.confirm', $t->id) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="button" onclick="event.stopPropagation(); confirmAction(event, this.closest('form'), 'Konfirmasi Transaksi #{{ $t->id }}?', 'Menyetujui transaksi ini akan mengubah statusnya menjadi Lunas.', 'success')"
                                            ...
                                            <form action="{{ route('admin.transactions.reject', $t->id) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="button" onclick="event.stopPropagation(); confirmAction(event, this.closest('form'), 'Tolak Transaksi #{{ $t->id }}?', 'Stok kursi dari pemesanan ini akan segera dikembalikan.', 'warning')"
